...
<body>
    <div class="div_head">
        订单列表
        <span>
              <?php if($add_order != '0'): ?><a style="text-decoration: none;color:#FA7124;" href="<?php echo U($add_order,array('p'=>$get['p'],'pid'=>$get['pid']));?>">【新建订单】</a>
                  <?php else: ?>
                  <a style="text-decoration: none;color:#FA7124;" href="javascript:alert('暂无权限')" >【新建订单】</a><?php endif; ?>
         <a href="<?php echo U('Admin/Student/school_apply',array('pid'=>$get['pid']));?>" style="width: 10px;padding:3px;height: 20px;
              text-decoration: none;margin-left:50px;font-weight: bold">驾校预约列表</a>
         <a href="<?php echo U('Admin/Language/language_apply',array('pid'=>$get['pid']));?>" style="width: 10px;padding:3px;height: 20px;
              text-decoration: none;margin-left:50px;font-weight: bold">语言教育预约列表</a>
        <a href="<?php echo U('Admin/Order/pay_list',array('pid'=>16,'p'=>$get['p']));?>"  class="span1"
           style="margin-left:100px;color: #FA7124;display: none">消息中心：<?php echo ($count['count1']); ?>条支付宝支付并未处理</a>
         <a href="<?php echo U('Admin/Order/order_list',array('pid'=>16,'p'=>$get['p'],'order_status'=>2));?>"  class="span1"
            style="margin-left:30px;color: #FA7124;display: none"><?php echo ($count['count2']); ?>条待回访</a>
        <a href="<?php echo U('Admin/Order/order_list',array('pid'=>16,'p'=>$get['p'],'order_status'=>3));?>" class="span1"
           style="margin-left:30px;color: #FA7124;display: none"><?php echo ($count['count3']); ?>条待结算需回访</a>
            ...
            <form action="?" id='form1' method="get" name="form1">
                <div>
                    ...
                    <span style="padding-left: 5px">
                        跟单客服：<input type="text" name="customer" style="width: 80px"  value="<?php echo ($get['customer']?$get['customer']:''); ?>"/>
                    </span>
                    <span style="padding-left: 5px">
                        驾校简称：<input type="text" name='s_nickname' style="width: 100px"  value="<?php echo ($get['s_nickname']?$get['s_nickname']:''); ?>" />
                    </span>
                    <span style="padding-left: 5px">
                        订单号：<input type="text" name='ordcode' style="width: 100px"  value="<?php echo ($get['ordcode']?$get['ordcode']:''); ?>" />
                    </span>
                    <span style="padding-left:5px">
                        下单时间：<input type="text" style="width: 120px;font-size: 12px"  onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',readOnly:true})" value="<?php echo ($get['create_time1']?$get['create_time1']:''); ?>" name="create_time1"/>
                        至
                        <input type="text" style="width: 120px;font-size: 12px" value="<?php echo ($get['create_time2']?$get['create_time2']:''); ?>"  onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',readOnly:true})" name="create_time2"/>
                    </span>
                </div>
                <div>
                    <span>
                    ...
                    <span style="padding-left: 5px">学员姓名：<input type="text" name='truename' style="width: 80px"  value="<?php echo ($get['truename']?$get['truename']:''); ?>"/></span>
                    <span style="padding-left: 5px">
                        学车基地：<input type="text" name='trainaddress' style="width: 100px"  value="<?php echo ($get['trainaddress']?$get['trainaddress']:''); ?>"/>
                    </span>
                    <span style="padding-left:5px">手机号：<input type="text" name='tel' style="width:100px"  value="<?php echo ($get['tel']?$get['tel']:''); ?>"/></span>
                     <span style="padding-left:5px">
                         支付时间：<input type="text" style="width: 120px;font-size: 12px"  onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',readOnly:true})" value="<?php echo ($get['notify_time1']?$get['notify_time1']:''); ?>" name="notify_time1"/>
                        至
                        <input type="text" style="width: 120px;font-size: 12px" value="<?php echo ($get['notify_time2']?$get['notify_time2']:''); ?>"  onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',readOnly:true})" name="notify_time2"/>
                    </span>
                </div>
                <div>
                    ...
                    <span style="margin-left: 5px">
                        回访时间：<input type="text" style="width: 120px;font-size: 12px"  onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',readOnly:true})" value="<?php echo ($get['return_time1']?$get['return_time1']:''); ?>" name="return_time1"/>
                            至
                        <input type="text" style="width: 120px;font-size: 12px" value="<?php echo ($get['return_time2']?$get['return_time2']:''); ?>"  onClick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss',readOnly:true})" name="return_time2"/>
                    </span>
                    <span style="padding-left:5px;">
                        <input value="查询" style="font-size: 11px" type="submit" id='btn' onclick="submitYouFrom('<?php echo U("Admin/Order/order_list",array('pid'=>$get['pid'],'p'=>$get['p']));?>')"/>
                        <input value="导出" style="font-size: 11px" type="button" id="button" onclick="submitYouFrom('<?php echo U("Admin/Order/push",array('pid'=>$get['pid'],'flag'=>0));?>')"/>
                    </span>
                </div>
    ...
    <table class="table_a"  width="100%">
        <tbody>
            ...
            <?php if(is_array($arr['list'])): $k = 0; $__LIST__ = $arr['list'];if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$v): $mod = ($k % 2 );++$k;?><tr>
                    ...
                    <td> <?php if($v['order_status'] == 1): ?>待处理<?php endif; ?>
                        ...
                        <?php if($v['order_status'] == 6): ?>已退款<?php endif; ?></td>
                    <td><?php echo ($v["customer"]); ?></td>
                    <td><?php echo ($v["customer_inform"]); ?></td>
                    <td><?php echo ($v["return_time"]); ?></td>
                    <td><?php echo ($v["lastupdate"]); ?></td>
                    <td>
                        <?php if($list_info != '0'): ?><a class="tablelink" href="<?php echo U($list_info);?>?id=<?php echo ($v['id']); ?>&pid=<?php echo ($get['pid']); ?>&p=<?php echo ($get['p']); ?>">
                                处理
                            </a>　
                            <?php else: ?>
                            <a class="tablelink" href="javascript:alert('暂无权限');">处理</a>　<?php endif; ?>
                    </td>
