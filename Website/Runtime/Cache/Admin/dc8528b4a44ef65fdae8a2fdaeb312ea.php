        ...
        <div style="width: 100%;text-align: center;">
            <?php if(is_array($school)): $i = 0; $__LIST__ = $school;if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$school): $mod = ($i % 2 );++$i;?><ul >
                    <li style="list-style: none;float: left;margin-left: 0px;text-align: center;width: 170px" >
                        <input type="radio" name="school_id" style="float: left;margin-top:5px" <?php echo ($info['id']==$school['id']?checked:''); ?> value="<?php echo ($school["id"]); ?>"/>
    ...
    $(function(){
        $('#submit').click(function(){
            $(this).attr('disabled',true);
                $.post("<?php echo U('Admin/Coach/school');?>",$('#form1').serialize(),function(res){
                    if(res.status){
