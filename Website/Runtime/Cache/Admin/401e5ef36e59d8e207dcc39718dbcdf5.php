...
<html>
    <head>
        ...
        <style>
            .dfinput{margin-left: 10px; border:solid 1px rgb(220,220,220);border-radius: 3px;width: 340px;padding: 10px 0 10px 5px}
            b{color: #FA7124}
        </style>
    </head>
    <body>
        <div class="div_head">教练分类添加</div>
        <div style="margin: 10px 15px;" >
            <div style="width:30%;">
                <form action="#" method="post" id="form1" enctype="multipart/form-data">
                    ...
                    <label style="padding: 10px 10px 20px 0" >分类名称<b>*</b></label>
                    <input type="text" name="category_name" class="dfinput" value="<?php echo ($category_name?$category_name:''); ?>" placeholder="请填写分类名称" style="padding: 5px 2px"/><br>
                    <input style="margin:20px 72px;width: 137px;height: 35px;line-height: 35px;background-color:rgb(19, 181, 177);border: none;border-radius: 4px;color: #ffffff "
                           type="submit" id="submit" value="<?php echo ($btn?$btn:'添加'); ?>"/>
                </form>
            </div>
            ...
            <div style="width:30%;margin: 20px 15px">
                <table class="table_a">
                    ...
                    <?php if(is_array($category)): $k = 0; $__LIST__ = $category;if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$data): $mod = ($k % 2 );++$k;?><tr>
                            ...
                            <td>
                                <a class="tablelink" href="<?php echo U('Admin/Coach/category_index',array('id'=>$data['id'],'pid'=>$get['pid']));?>">编辑</a>
                                <a class="tablelink" href="javascript:void(0);"
                                   onclick="del_category(<?php echo ($data['id']); ?>,<?php echo ($get['pid']); ?>,'jl')">删除</a>
                            </td>
                            <script>
...
<script>
    $("#submit").click(function(){
        ...
        $.post('<?php echo U("Admin/Coach/category_index");?>',$('#form1').serialize(),function(res){
            if(res.status){
