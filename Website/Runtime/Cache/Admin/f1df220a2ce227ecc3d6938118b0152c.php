...
<head>
    ...
    <style type="text/css">
        ...
        div.error{ position: absolute; font-size: 12px; color: #FA7124;}
        div.ok{ position: absolute; font-size: 12px;color: #38D63B;}
    </style>
    ...
    <script language="javascript">
        $(function(){
            ...
            var validate=$('#form1').validate({
                rules:{
                    username:{required:true, rangelength:[5,12],
                        remote:{url:"<?php echo U('chkAdmin');?>", type:'post'}
                    },
                    password:{required:true, rangelength:[5,12]},
                    verify:{required:true, remote:{url:"<?php echo U('chkVerify');?>", type:'post'}}
                },
                messages:{
                    username:{required:'用户名不能为空！', rangelength:'用户名长度必须在5到12位之间！', remote:'用户名不存在！'
                    },
                    password:{required:'密码不能为空！', rangelength:'密码长度必须在5到12位之间！'},
                    verify:{required:'验证码不能为空!', remote:'验证码不正确!'}
                },
