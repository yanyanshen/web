...
<html>
    ...
    <script language="javascript">
        $(function(){
            var validate=$('#form1').validate({
                rules:{
                    cd_tel:{required:true,cd_tel:true,remote:{url:'<?php echo U("Mobile/Login/reg_checkPhone");?>',type:'post'}},
            ...
            // 手机号码验证
                $("#add_jx").click(function(){
                    ...
                    if(validate.form()){
                        $.post("<?php echo U('Admin/Student/add_stu');?>",$("#form1").serialize(),function(res){
                                if(res.status){
    ...
    <body>
        ...
        <div style="margin: 10px 5px">
            <form action="<?php echo U('Admin/Student/add_stu');?>" id="form1" method="post" enctype="multipart/form-data">
                <ul style="padding:0;margin-left: 15px">
                    <li>
                        <label>用户账号<b>*</b></label>
                        <input type="text" name="cd_tel" class="dfinput"/><span style="color:red;"><?php echo ((isset($errorInfo["account"]) && ($errorInfo["account"] !== ""))?($errorInfo["account"]):""); ?></span>
                    </li>
                    <li>
                        <label>用户名称<b>*</b></label>
                        <input type="text" name="truename" class="dfinput"/><span style="color:red;"><?php echo ((isset($errorInfo["nickname"]) && ($errorInfo["nickname"] !== ""))?($errorInfo["nickname"]):""); ?></span>
                    </li>
                    <li>
                        <label>登录密码<b>*</b></label>
                        <input type="password" name="pass" class="dfinput" value="244ac348537069c3bfe9d633349b7334" /><span style="color:red;"><?php echo ((isset($errorInfo["pass"]) && ($errorInfo["pass"] !== ""))?($errorInfo["pass"]):""); ?>(默认为517xueche)</span>
                    </li>
                    <li>
